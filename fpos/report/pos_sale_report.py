...
class pos_sale_report(osv.osv):
    _name = "pos.sale.report"
    _description = "Sale Statistics"
    _auto = False
    _rec_name = 'date'
    ...
    def _select(self):
        select_str = """
            SELECT ROW_NUMBER() OVER (ORDER BY sub.id) AS id, sub.date, sub.product_id, sub.partner_id, sub.country_id,
    ...
    def _pos_sub_select(self):
        select_str = """
          SELECT
            MIN(l.id) AS id, 
            fo.date::timestamp::date AS date,
            l.product_id, partner.id AS partner_id, NULL AS payment_term, NULL AS period_id,
            u2.name AS uom_name,
            fo.currency_id, o.sale_journal AS journal_id, NULL AS fiscal_position, fo.user_id, fo.company_id,
            count(l.*) AS nbr,
            CASE WHEN fo.st = 'cancel' THEN 'receipt_cancel' ELSE 'receipt' END AS type,
            CASE WHEN fo.state = 'draft' THEN 'draft' ELSE 'paid' END AS state, 
            pt.categ_id, fo.date::timestamp::date AS date_due, NULL AS account_id, NULL AS account_line_id,
            NULL AS partner_bank_id,
            SUM(l.qty / u.factor * u2.factor) AS product_qty,
            SUM(l.subtotal) AS price_total,  
            CASE WHEN SUM(l.qty / u.factor * u2.factor) != 0 THEN SUM(l.subtotal) / SUM(l.qty / u.factor * u2.factor) ELSE 0 END AS price_average,
            0 AS residual,
            ...
            JOIN currency_rate cr ON
                (cr.currency_id = sub.currency_id AND
                 ...
                 (cr.date_end IS NULL OR cr.date_end > COALESCE(sub.date, NOW())))
                    ...
                    self._pos_sub_select(), self._pos_from(), self._pos_group_by() 
                ) )
